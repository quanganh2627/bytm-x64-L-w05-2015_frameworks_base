...
void main() {
    vec4 color = texture2D(UNI_Tex0, varTex0);
    ...
    vec3 r = vec3(0.95, 0.0, 0.2);
    vec3 g = vec3(0.2, 0.95, 0.0);
    vec3 b = vec3(0.0, 0.2, 0.95);
    int channel = int(floor(mod(gl_FragCoord.x, 3.0)));
    vec4 crt = vec4(r[channel], g[channel], b[channel], 1.0);
